...
public class Parkoló implements Comparable<Parkoló> {
    ...
    @Override
    public int compareTo(Parkoló p) {
        if(Integer.compare((this.kapacitás-this.rendszám), (p.kapacitás-p.rendszám))==0) {
            return this.cím.compareTo(p.cím);
        }
        return -1*Integer.compare((this.kapacitás-this.rendszám), (p.kapacitás-p.rendszám));
    }
}